...
Options:
  ...
  -e, --show-effects <SHOW_EFFECTS>  Show transaction effects [possible values: true, false]
      ...
      --skip-artifacts               Skip writing replay artifacts (transaction data, effects, gas
                                     report, cache summary, move call info, trace) to disk. Useful
                                     for performance profiling where artifact serialization would
                                     otherwise dominate the trace
  -n, --node <NODE>                  Network or GraphQL URL to replay against. Accepts `mainnet`,
                                     `testnet`, or a full GraphQL URL (e.g. for devnet or a
                                     self-hosted endpoint). When omitted, the network is derived
                                     from the active wallet env
