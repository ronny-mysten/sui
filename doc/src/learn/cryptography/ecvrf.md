...
Anyone can verify the proof using the public key corresponding to the secret key, so you can use it as a random number generator (RNG), which generates outputs that anyone can verify and applications that need verifiable on-chain randomness can use.
...
You can use any implementation following the same specifications with suite string `sui_vrf` (see section 5 in the [VRF specs](https://datatracker.ietf.org/doc/draft-irtf-cfrg-vrf/15/)) to compute VRF output and generate proofs.

The [fastcrypto](https://github.com/MystenLabs/fastcrypto) library provides a CLI tool for such an implementation, which the following example uses. 
...
This outputs a secret key and a public key in hex format. The secret and public keys are 32-byte strings:
...
This should create the 80-byte proof and VRF 64-byte output, both in hex format:
...
You can also verify it using the fastcrypto CLI tool:
...
The output provides confirmation:
